...
**Phase 2 status: complete.** All 108 checklist items done. Full suite: `cd api && npx jest` → 149 test suites (41 module-wiring + 108 unit specs), 1072 tests, all passing. Five real (pre-existing, non-blocking) production bugs were found along the way and documented — see [Findings](#findings--open-questions) — deliberately not fixed, since tests assert current behavior rather than intended behavior.
...
- `integrations/stripe/services/stripe-accounts..service.ts` — `getConnectedBalance`/`listBalanceTransactions`/`listCharges`/`listPayouts` all call the Stripe SDK inside `try { return this.stripe.X.list(...); }` without `await`, so a rejected promise is returned as-is to the caller — the `catch (error) { throw new error; }` (itself buggy: `new error` throws `TypeError: error is not a constructor`) never actually runs for async rejections. Net effect: the original Stripe error propagates unwrapped. Not fixed — tests assert current behavior with an explanatory comment so a future fix shows as an intentional test change.
- `integrations/storage/gcs/gcs.adapter.ts:42-72` — `uploadImage`'s write-stream `error` event rejects the returned Promise directly (asynchronously, via the event callback), bypassing the method's outer `try/catch` entirely — so a stream error propagates as the RAW error, not wrapped as `"Failed to upload image: ..."` like every other error path in the same method. Not fixed — test asserts current behavior.
- `modules/auth/controllers/email.controller.ts` — `registerWithEmail` wraps `return this.authService.registerWithEmail(dto)` in a try/catch with an empty catch body, but the call isn't `await`ed, so the catch never runs — a rejected promise just propagates normally to Nest's exception filter. Functionally harmless, but the empty catch is dead code that misleadingly implies errors are swallowed here.
- `integrations/stripe/services/stripe-payments-webhooks.service.ts` — two `switch` cases in `handleStripeWebhook` are missing a trailing `break`: `'customer.updated'` falls into `'payment_method.detached'` (harmless, both bodies are commented out); `'charge.failed'` (when its early inner `break` isn't hit) falls into `'charge.updated'`'s *live* code, unintentionally calling `stripePaymentsService.getBalanceTransaction(...)` on a charge.failed event. Not fixed — asserted as current behavior.
- `integrations/stripe/services/stripe-products.service.ts` — `getPrice` returns `unit_amount` in raw cents, while `createPrice`/`listPrices` both divide by 100 before returning. Inconsistent unit across the same service's public methods. Not fixed — asserted as current behavior.
