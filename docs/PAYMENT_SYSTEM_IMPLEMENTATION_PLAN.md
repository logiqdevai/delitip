...
> **⚠️ The paragraph above describes the pre-implementation state (2026-08-30). It is now out of date — see [§0 Implementation Status](#0-implementation-status-2026-09-01) below for what's actually built.** The rest of this document is kept as-is as the original audit/research trail; §0 is the only section that reflects current reality.

---

## 0. Implementation Status (2026-09-01)

**Phases 0–4 are implemented** on the `payments` branch, covering real Viva-backed tip collection, IBAN payout accounts, manual payout execution, and refund wiring — i.e., everything this document scoped for V1 (Option B, IBAN Bank Transfer, no Marketplace/connected accounts). Backend: 168 test suites / 1233+ tests passing, `tsc --noEmit` clean. Frontend: `tsc --noEmit`, lint, and `next build` all clean.

### What's built

- **`VivaIntegrationModule` is now imported into `AppModule`** — no longer dead code.
- **`PlatformFinanceConfig`** (`api/src/shared/config/platform-finance/`) — houses `TIP_PLATFORM_COMMISSION_PERCENTAGE`, `TIP_PROCESSOR_FEE_ESTIMATE_PERCENTAGE`, `PAYOUT_HOLD_WINDOW_HOURS`, matching §8.
- **`POST /public/tips`** — no longer completes synchronously. Creates `Tip(status=CREATED)` + `PaymentTransaction`, opens a real Viva Smart Checkout order (with a client-supplied `client_request_id` idempotency key), and returns `{ tip_id, checkout_url }` for the frontend to redirect to. **`GET /public/tips/:id/status`** and **`GET /public/tips/by-order-code/:orderCode`** (new, not in the original plan — a fallback lookup for the checkout-return page when `sessionStorage` isn't available, e.g. a different device) are both live.
- **`api/src/modules/payments/`** (new module) — `POST /webhooks/viva` + `GET /webhooks/viva` (handshake), processed **inline** in the HTTP handler rather than via BullMQ (a deliberate scope simplification from §7's original queued-processing design). Idempotent via `WebhookEvent.message_id`. Handles `1796`/`1797`/`1798`/`1799`/`768`/`769`. `TipDistribution` rows are created here, at payment confirmation, not at tip-creation time, exactly as §9 specifies. A scheduled reconciliation sweep (`@nestjs/schedule`, `EVERY_10_MINUTES`, plus a manual `POST /admin/payments/reconcile`) covers abandoned/expired orders — also inline rather than BullMQ, matching the same simplification.
- **IBAN payout accounts** — `POST /stores/:storeId/payout-account` and `POST /users/me/payout-account` now require `{ iban, beneficiary_name, friendly_name? }`, validate the IBAN (mod-97 checksum, `api/src/shared/utils/iban/`), call `VivaBankTransfersService.linkBankAccount`, and persist only `bank_account_id` + masked `iban_last4` — never the raw IBAN, per §16. The mock free-form status-override `PATCH` is gone.
- **`api/src/modules/payouts/`** (new module) — `POST /stores/:storeId/payouts/run` (manual "Pay out now," no cron batch job — the other deliberate scope simplification), atomic distribution claiming, real `VivaBankTransfersService` fee-quote + `:send` execution, `GET /stores/:storeId/payouts` and `GET /employees/:id/payouts` history. Bundles a store's own share with all its employees' eligible distributions in one run (§5.1 rationale documented in code).
- **Refunds** — `RefundsService.update()` now calls `VivaTransactionsService.createFastRefund`/`createRebate` (same-day vs. previous-day, per §2.5) before finalizing locally, and flags `requires_manual_reconciliation` when distributions were already paid out (§10.4).
- **Frontend** — full-page redirect checkout flow (`review-step.tsx` → Viva → `/checkout/return` → back into the store's own tip flow at `?tip=<id>`, which polls status and renders processing/success/failure states via a new `CheckoutStatusStep`); real IBAN forms (shared `IbanPayoutAccountDialog`) for both store and employee payout accounts; a new Payouts history page + sidebar entry; "Pay out now" button with held/eligible breakdown; financial-breakdown panel on tip detail (OWNER/ACCOUNTANT-gated).
- **Database** — all schema changes from §11 applied via migrations (`PaymentTransaction`, `WebhookEvent`, `Payout`, `PayoutAccount`/`Refund`/`Tip` extensions). Note: `PaymentTransaction.viva_order_code`/`viva_transaction_id` were subsequently renamed to **`provider_order_code`/`provider_transaction_id`** (provider-generic naming, since `PaymentTransaction.provider` already supports non-Viva processors) — any references to the old names elsewhere in this doc are stale.

### Deviations from this document (all deliberate, user-approved scope decisions)

1. **No BullMQ.** Webhook processing and the reconciliation sweep are inline/cron (`@nestjs/schedule`) rather than queued, per §7's alternative. `api/src/core/queues/**` remains provisioned but unused.
2. **No scheduled payout batch job.** Only the manual `POST /stores/:storeId/payouts/run` trigger exists — no daily cron batch (§10.3's "Scheduled BullMQ job (daily default)" was not built).
3. **Marketplace/connected accounts (Option A) — explicitly out of scope**, confirmed by the user. IBAN Bank Transfer (Option B) is the only payout method.

### Discovered during implementation, not anticipated by the original audit

- **Viva issues two separate OAuth2 client credential pairs** for the sandbox account — "Smart Checkout" and "Account Transactions" — each authorized for a different permission group, not one pair for the whole merchant account as §2.1/§16 assumed. Confirmed empirically: the Smart Checkout client gets `403` on `/banktransfers/v1/*` and `/merchants/v1/wallets`. Fixed by extending `VivaAuthService`/`VivaConfig`/`VivaHttpClient` to cache and request a token per scope (`VivaOAuthScope.CHECKOUT` / `.ACCOUNT_TRANSACTIONS`), with `VIVA_ACCOUNT_TRANSACTIONS_CLIENT_ID`/`_SECRET` as new env vars. `VivaBankTransfersService` and `VivaWalletsService` now request the Account Transactions scope explicitly.
- **`Tip.selected_employee_ids`** (new `Json?` column, not in §11) was added — since `TipDistribution` creation moved to webhook-confirmation time (§9), the webhook handler needs to know which employee(s) the customer selected at checkout time (relevant for `CHOOSE_MANY`/`TEAM` QR modes), which is no longer implicit at tip-creation time the way it was before.
...
1. **PSD2 "intermediary" posture** — Viva's own marketplace documentation states that a platform acting as an intermediary between customers and sellers should use the Marketplace solution for compliance. Delitip's Bank Transfer-based V1 design is structurally an intermediary. Confirm with Viva/legal whether the Bank Transfer API is an acceptable long-term posture or whether Marketplace onboarding becomes a compliance requirement at scale. **Update 2026-09-01: see `docs/VIVA_MARKETPLACE_MIGRATION_RESEARCH.md` for a full migration research pass — Viva's docs now state this requirement unconditionally (not hedged), and the Marketplace SDK wrapper (`VivaMarketplaceService`) is already fully built in this codebase, just unwired. Commercial access (Sales/Live Chat) remains the actual blocker.**
2. **Natural-person payee onboarding** — no public Viva documentation confirms whether an individual (non-business) can be onboarded as a Marketplace connected account. This affects only the Phase 5/Option-A future path, not the recommended V1 Bank Transfer approach. **Update 2026-09-01: Viva's docs now confirm KYC/AML is handled for both businesses and individuals — see `docs/VIVA_MARKETPLACE_MIGRATION_RESEARCH.md` §4 for detail and the residual UX-cost caveat (an employee must complete Viva's own hosted onboarding, not just hand over an IBAN).**
