...
- [x] **MVP end-to-end demo possible:** sign up → add employee → rule → QR → tip → see tip/review in dashboard & employee portal
  - **Live-verified 2026-08-29** against a real dev DB: the entire loop was click-tested end-to-end via browser automation (see the Manual E2E script section) — sign up → onboarding → add employee → distribution rule → QR code → live tip flow → review → visible in Tips Ledger, Reviews, Overview, and Analytics. Zero frontend bugs. Caveat: "see it in the employee portal" specifically required a manual DB workaround to log in as the employee, since real employee accounts can't currently be claimed at all (see the new Blocked entry) — the dashboard half of the loop is fully real end-to-end with no workaround needed.
...
- [x] Mobile/responsive pass for dashboard, employee, tip flow
  ...
  - Live-verified (2026-08-29): resized the browser to a 390×844 mobile viewport (via chrome-devtools MCP) and checked Dashboard Overview, Tips Ledger, and Settings against a real, populated org. All three render correctly with no page-level horizontal overflow: the sidebar collapses into a horizontally-scrollable pill tab bar, cards stack full-width, and the Tips Ledger table scrolls within its own container rather than the page. Only cosmetic artifact found: the floating Next.js DevTools "N" indicator button (dev-mode only, absent from production builds) overlaps card content at this width — not a real app bug, not worth fixing. No further action needed.
