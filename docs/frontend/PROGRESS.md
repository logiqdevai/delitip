...
**Overall Progress:** 78 / 87 tasks completed (Phases 0–10, now fully and precisely recounted — see the table below; +1 from the 2026-08-29 live-testing round flipping Phase 7's responsive pass from static review to live-verified)

**Product state:** Every engineering task reachable without external inputs is done, **and the entire MVP core loop plus most of Post-MVP has now been live-verified** (2026-08-29) against a real external Postgres dev DB the user supplied, driven end-to-end via browser automation — not just `tsc`/`next build`/lint plus reading code, as it was earlier this session. Zero frontend bugs were found in the live run. That same live-testing round also found two real **backend** bugs (employee accounts could never be claimed/logged into; a refunded tip's distribution was never cancelled, double-counting it in payout/earnings totals) — both were then **fixed and live-verified the same session**, at the user's explicit request; see "Resolved this session" right after the Blocked table for the full writeup, and the Manual E2E script section below Phase 9 for the original test results.

**Later the same day**, at the user's request, every backend endpoint was cross-referenced against the frontend `features/` layer to find any not wired up via TanStack Query. Found and fixed five real gaps (distribution rule delete — plus a backend safety guard it was missing entirely, QR code stats display, logo/cover image removal, per-language translation editing, and a customer-facing self-service refund request) and two incidental bugs (a case-sensitivity bug that made saved branding messages look like they never persisted; a stores query-key bug that could leave stale data in an open detail view after any Store update). All fixed, unit-tested, and live-verified; see "API / TanStack Query completeness audit" right after "Resolved this session" for the full writeup. What remains is only:

- **Needs something this session genuinely cannot produce**: a real payment gateway account (Phase 10, and the mock-payment note under 2.6/5.4 — the user has explicitly said to leave this for now), an error-monitoring account/DSN (Phase 10), counsel-approved legal copy (Phase 10, pre-existing), a product decision on the `/me` vs `/customer` customer-history route (6.3, pre-existing), translated UI copy for a tip-flow language switcher plus industry-specific review/feedback templates (6.1), and a handful of lower-severity pre-existing backend items in the Blocked table (over-shared `User` fields on two endpoints, the public-review-redirect message/threshold coupling, etc.) that weren't part of what the user asked to be fixed this round.

Everything else in Phases 0–10 is checked off. The two open MVP items are `0.3` (a standing practice, not a one-time completable task) and `3.4`'s `/employee/qr` (genuinely blocked — no API path exists for an employee session to find their own QR code). Phase 11's "Finished Product" checklist above now reflects both the live-verification results and the two bug fixes: "Core loop works end-to-end in production", "MVP E2E verified", and "Employee portal works for real staff accounts" are all checked now that a real employee can actually claim an account and reach it.

**Next Task:** No purely frontend-engineering task remains in this environment. The remaining Blocked-table items are either external-input blockers (payment gateway — explicitly deferred by the user for now, error monitoring, legal copy, customer-history route decision, i18n content) or lower-severity backend items nobody has asked to be fixed yet (User-field over-sharing on Refunds/Members endpoints, the review-redirect message/threshold coupling).

**Next Documentation:** This file's own Manual E2E script (under Phase 9, now with live results) and the Blocked table (end of file), plus the "Resolved this session" and "API / TanStack Query completeness audit" writeups right after it.

**Progress-count correction (2026-08-29):** Every phase (0 through 10) was recounted precisely against the file's actual top-level checkboxes this session, replacing every earlier rough estimate (the original plan's ~112 total, and this session's own intermediate ~109 estimate). See the table below — 87 is now an exact count, not an approximation. The later live-testing round mostly verified already-"complete" work and surfaced two new backend bugs rather than finishing new frontend tasks, but it did flip one genuinely open item: Phase 7's responsive pass went from "static review only" to actually live-verified at a mobile viewport, moving Overall from 77 to 78.
...
Note: Earlier in this session, this environment's Docker Desktop could not hold a Postgres container up long enough to run migrations + seed data, so verification relied on `tsc`/`next build`/lint plus reading the actual NestJS controllers/services/DTOs. That gap was closed 2026-08-29 when the user supplied a real external Postgres dev DB and explicit authorization to use it for testing — see the Manual E2E script section for the live results this unlocked.
...
**No purely frontend-engineering task remains actionable** without a product/backend decision or external accounts/content. In priority order:

1. **Decide how to fix the two new live-verified backend bugs** (both in the Blocked table, added 2026-08-29): employees can never claim/log into their account (blocks the entire Employee Portal for real users — likely the higher-priority one), and refunded tips still show as owed to employees in payout totals. Both need a backend engineering pass once the approach is decided; neither is a frontend fix.
2. **Resolve external/content blockers** as they become available: a payment provider decision + credentials (Phase 10 payment items and, longer-term, real employee cash-out — **the user has explicitly said to leave this for now**, do not restart this without being asked), an error-monitoring account (Phase 10), legal copy from counsel (Phase 10), a product decision on the customer-history route (6.3), and translated tip-flow copy + industry review/feedback templates (6.1).
3. **If none of the above is available**, the next purely engineering task in phase order would be **Phase 6.3 Customer account history** — but it's explicitly blocked on the `/me` vs `/customer` route decision (see Blocked table), so don't start it without that decision first; skip to whatever Blocked item resolves first.
...
- [x] Set Store default distribution rule
  ...
  - Update (2026-08-29, later the same day): Delete is no longer deferred — see the "API / TanStack Query completeness audit" section after the Blocked table. `DistributionRulesService.remove()` gained a real safety guard (blocks deleting a rule used by a past tip, set as the Store default, or assigned to a QR code) and the Distribution hub gained a "Delete" button + `ConfirmationDialog`, both live-verified.

### Distribution — Feature completion `(MVP)`

- [x] CRUD for rules — delete implemented with a backend safety guard (in-use rules can't be deleted), live-verified
...
- [x] Live route works end-to-end in sandbox
  - Verified: typecheck (`tsc --noEmit`) clean, `next build` clean. **Live-verified end-to-end 2026-08-29** against a real dev DB via chrome-devtools MCP browser automation: the full interactive wizard (employee selection → amount → pay → thank-you → review) was click-tested against a real API + seeded Postgres, not just read from code — every step rendered correctly, the §29 disclosure text showed on the payment step, and the review submission correctly reached the dashboard. See the Manual E2E script section (step 6) for full results.
...
- [x] Wire `/dashboard/tips` to API
  ...
  - Notes (2026-08-29): `app/dashboard/tips/components/tips-page-content.tsx`, `useStoreTips` (already scaffolded in 0.2). Status/date filters map straight onto `TipsQuerySchema` (`status`, `date_from`, `date_to`); employee/QR filters left as the doc's noted nice-to-have. Status chips use the real `getTipStatusLabel`/`TipStatusFilterOptions` (`PENDING`/`COMPLETED`/`FAILED`/`REFUNDED`) — no invented "Settled" state. Extended `features/tips/interfaces.Tip` with `employee`/`qr_code`/`distributions` refs to match `TipsService.findAll`'s Prisma `include`. Removed the now-dead `demoTips` export from `dashboard-demo.ts` (only consumer was the old table). Export CSV button renders disabled with a title tooltip explaining why, per doc.
  - Live-verified (2026-08-29): confirmed against a real DB — the ledger listed a real completed tip with correct employee/QR/amount/provider columns, and its status chip correctly flipped from `Completed` to `Refunded` after the refund lifecycle (5.4) completed. Tip detail page (4.1) opened from the ledger row and rendered the correct Summary/Distribution/Review/Refunds sections.
...
- [x] Wire `/employee` earnings to API
  ...
  - Live-verified (2026-08-29): `/employee` correctly showed a real tip in "Today's Tips" and the month total after signing in via `/auth/sign-in?role=employee` (initially had to manually set a password in the DB to reach it at all, since real employees couldn't claim an account yet — that gap and the earnings-total data-accuracy issue below were **both fixed the same session**; see "Resolved this session" after the Blocked table). Confirmed post-fix: `EmployeesService.dashboard()`'s `tips_this_month.total_amount` correctly dropped to exclude a refunded tip's `TipDistribution` once its `payout_status` flips to `CANCELLED`.
...
- [x] Gate dashboard nav items by `OrganizationRole`
  ...
  - Notes (2026-08-29): `useWorkspace()` now exposes `role: OrganizationRole | null` from the active `organization_memberships` row (`GET /users/me/accounts`, not previously surfaced). `DashboardSidebar` filters Employees / Tip Distribution / QR Codes out of the nav for `ACCOUNTANT`, leaving Overview, Tips, Reviews, Analytics, Settings — a direct reading of this task's own criteria text; the *exact full* Accountant page set stays flagged as unclear in the Blocked table below, this only covers what the criteria explicitly named. "Store Manager scoped to Store" and "Employee never sees `/dashboard` nav" were already true structurally (`useWorkspace` only ever surfaces a Store-scoped member's own store; `AuthRouteGuard` fully redirects employee-only sessions out of `/dashboard/*`) — verified, not changed. Route-level 403 enforcement for hidden pages (someone typing the URL directly) is Phase 8, not this task.
...
- [x] **MVP end-to-end demo possible:** sign up → add employee → rule → QR → tip → see tip/review in dashboard & employee portal
  - **Live-verified 2026-08-29** against a real dev DB: the entire loop was click-tested end-to-end via browser automation (see the Manual E2E script section) — sign up → onboarding → add employee → distribution rule → QR code → live tip flow → review → visible in Tips, Reviews, Overview, and Analytics. Zero frontend bugs. Caveat: "see it in the employee portal" specifically required a manual DB workaround to log in as the employee, since real employee accounts can't currently be claimed at all (see the new Blocked entry) — the dashboard half of the loop is fully real end-to-end with no workaround needed.
...
- [x] Refunds + request from tip detail
  ...
  - Notes (2026-08-29): `api/src/modules/refunds` (manager-created + public customer-request + approve/reject/complete) is real and registered. New `features/refunds/` module. Tip detail (4.1) gained a "Request refund" button (shown when `COMPLETED` and no active PENDING/APPROVED refund already exists) opening `request-refund-dialog.tsx` — amount defaults to and is capped at the tip's full amount, matching `CreateRefundDto`. Refunds panel: Approve and "Mark completed" are plain actions (reversible-ish, not destructive per se — approving isn't final); Reject goes through `ConfirmationDialog` since it's a one-way terminal state (`RefundsService.update` blocks any further change once `REJECTED`/`COMPLETED`). **Noted, not fixed (backend, out of scope for a frontend-only pass):** `RefundsService.findAll`/`findOne` `include: { requested_by: true, processed_by: true }` pulls the full `User` row including the password hash, not a `select`-scoped subset — the frontend `Refund` interface only types the safe fields and never reads/renders `.password`, but the API response itself over-shares. Worth a backend fix.
  - Live-verified (2026-08-29): ran the full lifecycle end-to-end (Request refund on tip detail → Approve in the Payments hub queue → Mark completed) against a real DB. Every UI step, toast, and status transition worked exactly as designed, and `Tip.status` correctly ended at `REFUNDED`. Surfaced one backend data-accuracy bug in the process: the tip's `TipDistribution` row stayed `payout_status: PENDING` on the tip detail page and in the employee portal's earnings totals even after the refund fully completed (the Payments hub's "Pending distributions" panel was unaffected — it already excludes non-`COMPLETED` tips server-side). **Fixed the same session**: added a `CANCELLED` `PayoutStatus` value (migration `20260829144830_cancel_distributions_on_refund`) and `RefundsService.update` now cancels a tip's still-pending distributions in the same transaction that marks it `REFUNDED`. Re-ran the full lifecycle on a fresh tip post-fix and confirmed the Distribution row now reads "Cancelled". See "Resolved this session" after the Blocked table.
...
- [x] Alerts live
- [x] Spots live
- [ ] Payments/refunds/cash-out real — Payments hub + Refunds real; production employee cash-out stays blocked (no transfer endpoint exists anywhere in the API — see Blocked table)
...
- [x] Gate Post-MVP nav/features by `SubscriptionPlan`
  ...
  - Live-verified (2026-08-29): confirmed the full gate lifecycle against a real DB — a fresh Starter-plan org correctly saw the "Analytics needs the Professional plan" upsell with a working "Go to Billing →" link; upgrading via the Settings → Billing plan `NativeSelect` correctly and immediately flipped the org to Professional/ACTIVE (toast confirmed); navigating back to Analytics immediately after correctly rendered the real page instead of the upsell. No frontend bugs found in either direction of the gate.
...
- [x] Mobile/responsive pass for dashboard, employee, tip flow
  - Details: [navigation.md](./navigation.md), [customer-tipping.md](./customer-tipping.md)
  - Completion criteria:
    - [x] Tip flow usable on narrow viewports
    - [x] Dashboard usable on tablet/mobile (drawer acceptable)
  - Notes (2026-08-29): Every page built this session follows the existing codebase's responsive conventions (`sm:`/`lg:` grid breakpoints, `overflow-x-auto` on tables, the tip flow's `max-w-md` mobile-first shell).
  - Live-verified (2026-08-29): resized the browser to a 390×844 mobile viewport (via chrome-devtools MCP) and checked Dashboard Overview, Tips, and Settings against a real, populated org. All three render correctly with no page-level horizontal overflow: the sidebar collapses into a horizontally-scrollable pill tab bar, cards stack full-width, and the Tips table scrolls within its own container rather than the page. Only cosmetic artifact found: the floating Next.js DevTools "N" indicator button (dev-mode only, absent from production builds) overlaps card content at this width — not a real app bug, not worth fixing. No further action needed.
...
- [x] Responsive pass — live-verified 2026-08-29 at a 390×844 mobile viewport (see note above)
...
- [x] Manual E2E script documented for MVP loop (sign-up → tip → dashboard)
  ...
  - Notes (2026-08-29): Written below, then **executed live later the same day** once the user supplied a real external dev DB — see the updated Manual E2E script section for full results (all 8 steps passed, zero frontend bugs; two backend bugs found were fixed the same session — see "Resolved this session" after the Blocked table).
...
- [x] MVP E2E script executed and noted — executed live 2026-08-29 against a real dev DB; see the Manual E2E script section above for full step-by-step results

### Manual E2E script — MVP loop

Written 2026-08-29, **executed live 2026-08-29** against a real external Postgres instance supplied by the user (empty, migrations applied fresh) with both `api` (port 3000) and `app` (port 3001) running locally against it, driven end-to-end via chrome-devtools MCP browser automation. Result: **every step below passed with zero frontend bugs found.** Two real *backend* bugs were found in the process — neither was a frontend defect, and neither blocked completing the script itself — and **both were fixed and re-verified live the same session** at the user's request; see "Resolved this session" after the Blocked table for the fix details:

1. **Sign up** — `Routes.auth.sign_up`. Fill business name, email, password, business type, team size. Submit → expect redirect to `Routes.onboarding`. ✅ Passed.
2. **Onboarding** — confirm/adjust Store name, industry, timezone, currency. Submit → expect redirect to `Routes.dashboard.root`. Confirm the sidebar store chip shows the new Store. ✅ Passed.
3. **Add an employee** — `Routes.dashboard.employees` → "Add New Employee" → fill name/email → submit → expect it in the list with an active badge. ✅ Passed. (Discovered separately: the created employee couldn't log in with this email/password at all — the create flow itself worked exactly as designed; the gap was in the account-claim path, since fixed. See "Resolved this session".)
4. **Create a distribution rule** — `Routes.dashboard.distribution` → create a rule (e.g. 100% to the tipped employee) → set it as Store default → confirm the "Store default" panel reflects it. ✅ Passed.
5. **Create a QR code** — `Routes.dashboard.access` → create a QR with `CHOOSE_ONE` mode, assign the employee from step 3 → confirm the tip URL renders and the PNG download/print work. ✅ Passed (Spots panel also confirmed present and correctly empty).
6. **Live tip flow** — open the tip URL from step 5 in a new tab (simulating a guest scan). Confirm: store branding renders; the employee row is selectable; Continue → amount step shows the Store's suggested presets; Continue → payment step shows the confirm/pay summary and the §29 disclosure text; Pay → thank-you step shows the amount and employee name; Leave a review → submit a 5-star rating with a comment → confirm the done screen (public-redirect link if `public_review_redirect_url` is configured in Settings, private-thanks message otherwise). ✅ Passed, with one caveat: a 5-star review with no `public_review_redirect_url` configured showed the "sorry your experience wasn't perfect" copy and a Private badge instead of the public-prompt copy — this is the pre-existing, already-documented `ReviewsService.createPublic` backend bug ("Public review 'redirected' message copy" in the Blocked table, still open — not part of what was fixed this round), now live-confirmed rather than just inferred from reading the code.
7. **See it in the dashboard** — back in the business tab: `Routes.dashboard.tips` shows the new tip as `COMPLETED`; clicking it opens the tip detail with the correct distribution row; `Routes.dashboard.reviews` shows the new review; `Routes.dashboard.root` KPIs reflect it; the Analytics page's Overview tab reflects it (confirmed the Starter-plan upsell screen shows correctly first, then confirmed Analytics itself renders correctly immediately after upgrading the org to Professional via Settings → Billing — plan-gating and the mock billing upgrade both verified live). ✅ Passed. Also went beyond the script to live-test the full refund lifecycle from the tip detail page (Request refund → Payments hub Approve → Mark completed): the UI flow and toasts all worked correctly at every step, and `Tip.status` correctly ended at `REFUNDED` — but this surfaced the distribution-not-cancelled bug, since fixed and re-verified (see "Resolved this session").
8. **See it in the employee portal** — real sign-in as the employee was impossible at the time of this step (the account-claim bug, since fixed), so this was first verified via `/auth/sign-in?role=employee` after manually setting a password directly in the DB for test purposes only: `/employee` correctly showed the new tip in "Today's Tips" and the month total, `/employee/reviews` correctly showed the new review and 5.0 average rating, and `/employee/qr` correctly rendered its pre-existing "Preview only" labeled state (already documented, not a new finding). ✅ Passed for the pages themselves. After the account-claim fix, re-verified that a genuinely fresh shell user (mimicking a real "Add New Employee" row) can now claim its account through the real `POST /auth/email/register` endpoint and log in — no workaround needed anymore. See "Resolved this session" after the Blocked table.

None of the "expected failure points" flagged when this script was written (the `EmployeeTipDistribution`/`Tip` type distinction in `useEmployeeTips`, the Json-vs-string review-category/feedback-question fields, the Overview page's dependence on `organizations/:id/dashboard/overview`) actually failed — all three were confirmed working correctly live.
...
- [x] Core loop works end-to-end in production — **live-verified 2026-08-29** against a real dev DB (see Manual E2E script above): sign-up → onboarding → employee → distribution → QR → live tip flow → review → dashboard/analytics all confirmed working with zero frontend bugs. ("In production" caveat: this was a real Postgres instance and real HTTP round-trips, not a mocked test, but still local `next dev`/`nest start`, not an actual deployed environment)
- [x] Employee portal works for real staff accounts — pages live-verified working (`/employee`, `/employee/reviews`, `/employee/qr`) against real data. The account-claim bug found during that verification (an employee added via "Add New Employee" had no way to ever set a password and reach `/employee`) was **fixed the same session** — `registerWithEmail`/`forgotPassword` now let a passwordless shell account be claimed; live-verified via a fresh shell user successfully claiming an account through the real register endpoint. See "Resolved this session" after the Blocked table
...
- [x] MVP E2E verified — executed live 2026-08-29 against a real dev DB; see the Manual E2E script section for full results
- [ ] No known blocking issues in Blocked section (or all accepted) — the two **critical** items this session's live testing found (employees couldn't ever log in; refunded tips showed as still owed to employees) were fixed and live-verified the same session (see "Resolved this session"), but the Blocked table still has real, unresolved entries (payment gateway, error monitoring, legal copy, customer-history route decision, i18n content, and two lower-severity pre-existing backend items) that need a product decision or an external account this session cannot provide
...
### Resolved this session (2026-08-29)

Both found and fixed live, after the user asked for them to be fixed rather than just documented:

- **Employees could never claim/log into their account** — `EmailAuthService.registerWithEmail` (`api/src/modules/auth/services/email.service.ts`) now lets a request "claim" an existing passwordless/empty-password `User` row (updates it in place, sets `registered_at`) instead of always throwing `ConflictException`; it still rejects a genuine duplicate (an existing user who already has a password). `PasswordService.forgotPassword` (`api/src/modules/auth/services/password.service.ts`) no longer gates on `user.password` being truthy, so a shell user can receive a claim link too. Live-verified against the real dev DB: a fresh passwordless shell user (mimicking what "Add New Employee" creates) successfully claimed its account via `POST /auth/email/register` and could then log in; a second register attempt against the same now-claimed email correctly got a real Conflict; `POST /auth/forgot-password` against a still-unclaimed shell now actually creates a `PasswordResetToken` row (confirmed 0 → 1 in the DB), where before it silently no-opped. Backend test suites updated (`email.service.spec.ts`, `password.service.spec.ts`) and the full backend suite (149 suites / 1075 tests) still passes.
- **Refunded tips still owed the employee a "pending" payout** — added a `CANCELLED` value to the `PayoutStatus` enum (migration `20260829144830_cancel_distributions_on_refund`, additive/backward-compatible) and `RefundsService.update` now cancels any still-`PENDING` `TipDistribution` rows for a tip in the same transaction that flips it to `REFUNDED`. `EmployeesService.dashboard()`/`tips()` now exclude `CANCELLED` distributions, which also fixes the employee cash-out balance (it already filtered to `payout_status === "PENDING"` client-side, so cancelling the row was enough). Frontend `PayoutStatuses`/`PayoutStatusFormOptions` got the new `CANCELLED`/"Cancelled" value so the tip detail page's Distribution row labels it correctly instead of falling back to the raw enum string. Live-verified end-to-end with a fresh tip → refund → approve → mark-completed cycle: the tip detail page's Distribution row now reads "Cancelled" instead of "Pending", and the employee dashboard's `tips_this_month.total_amount` dropped to exclude it. Also ran a one-time backfill on this test DB's two pre-existing refunded tips (created before the fix) so they're consistent with the new behavior too. **Correction to this entry's original write-up**: the Payments hub's "Pending distributions" panel was never actually part of this bug — it queries tips with `status: "COMPLETED"` server-side, so a `REFUNDED` tip was always excluded from it automatically; only the tip detail page's Distribution row and the employee portal's earnings/balance totals were affected. Backend test suites updated (`refunds.service.spec.ts`, `employees.service.spec.ts`); full backend suite still passes (149/1075).

### API / TanStack Query completeness audit (2026-08-29, later the same day)

At the user's request, cross-referenced every backend endpoint (`api/src/modules/**/*.controller.ts`, ~30 controllers) against `app/src/features/**` to find any that had no frontend consumer at all (a dead endpoint) or a consumer that wasn't wired into any actual page/component (a built-but-unused hook). Found five real gaps and two related bugs; all fixed, tested (backend unit tests + `tsc` + `eslint` on both packages + `next build`), and live-verified against the real dev DB via browser automation:

- **Distribution rule delete had zero UI *and* zero backend safety guard** — `DELETE /distribution-rules/:id` existed and `useDeleteDistributionRule` was already built, but no page called it (matching this file's own honest "(delete rule deferred)" note from 2.4). Investigating why turned up a real bug: `DistributionRulesService.remove()` had no check at all before deleting — a rule already used by a past `Tip`, set as the Store's default, or assigned to a QR code could be silently deleted with no warning. Fixed `remove()` to reject deletion (400, with a specific reason) in all three cases, then added the "Delete" button + `ConfirmationDialog` to `distribution-rule-card.tsx`/`distribution-page-content.tsx`. Also fixed `distribution.services.ts` to surface the backend's real error message instead of a generic fallback (needed for the guard's message to actually reach the user). Live-verified both paths: deleting an in-use rule correctly showed the specific blocking reason in a toast with the dialog staying open; deleting a fresh, never-used rule correctly succeeded.
- **QR code stats were fetched by an unused hook** — `useQrCodeStats`/`GET /qr-codes/:id/stats` (tips count, tips total, reviews count) existed but no component ever called it. Typed the previously-`unknown` response as a proper `QrCodeStats` interface and added a small stats row to each QR card on `/dashboard/access`. Live-verified: correctly showed "3 tips · €0.00 · 1 review" for a QR whose 3 tips had all since been refunded — proving the backend's `status: COMPLETED` filter on `tips_total_amount` and the count-of-all-tips on `tips_count` both work correctly together.
- **No way to remove an uploaded logo/cover image** — `DELETE /documents/:id` existed but had no frontend consumer (only upload was wired). Added `useDeleteDocument` + a "Remove" button in `branding-settings-form.tsx` that deletes the Document and clears the Store's `*_document_id` field. Required widening `UpdateStorePayload.logo_document_id`/`cover_document_id` from `string` to `string | null` (they couldn't express "clear this" before). Live-verified (via a seeded test Document row, since GCS credentials aren't configured in this sandbox — a pre-existing, already-documented ops gap, not a code issue): Remove correctly deleted the row and the logo disappeared from the form immediately, no page refresh needed.
- **No way to hand-edit a Store's per-language welcome/thank-you translation** — `PATCH /stores/:id/translations/:field` existed (lets a manager fix one language's translation independent of the auto-translate-stub that just copies the primary-language text everywhere) but had no frontend consumer at all. Added the full stack (`updateStoreTranslation` service/hook, `StoreTranslatableField`/`UpdateStoreTranslationPayload` types) and a new "Per-language messages" section in `localization-settings-form.tsx` — one row per supported non-primary language with two textareas and a Save button. Live-verified: added Greek as a supported language, hand-edited both messages, confirmed two "Translation saved" toasts, and confirmed via a direct DB read that `welcome_message`/`thank_you_message` now hold `{en: "...", el: "..."}` correctly.
- **Customers had no self-service way to request a refund** — `POST /public/tips/:tipId/refund-request` (no account needed, per the endpoint's own Swagger summary referencing spec §28) existed but was entirely unwired — not in any service, hook, or component. Added `useCreatePublicRefundRequest` (no toast, matching the existing `useCreatePublicTip`/`useCreatePublicReview` convention for this full-screen guest flow) and a collapsible "Something wrong with this tip? Request a refund" section on the tip flow's `DoneStep` (the one screen every customer reaches regardless of whether they left a review). Live-verified end-to-end: submitted a request with no customer account from the public tip page, got the inline "Refund requested" confirmation, and confirmed it appeared correctly in the manager's Payments hub Refunds with the exact reason text, ready to Approve/Reject.

Two more bugs surfaced incidentally while building the above, both fixed:

- **`branding-settings-form.tsx` could never actually load a saved welcome/thank-you message** — it looked up `store.welcome_message?.[store.primary_language]`, but `primary_language` is the uppercase enum value (`"EN"`) while the translation map's keys are always lowercase (`"en"`, set by both `autoTranslateStub` and `updateTranslation`) — so the field always rendered empty even when a message was saved, making it look like every save silently failed. Fixed the lookup to lowercase the key first. Live-verified: saved "Welcome to our lovely bistro!", hard-reloaded the page, and confirmed it now reappears (and confirmed the public tip page itself renders it too).
- **Any Store update could leave stale data in an open detail view** — `storesQueryKeys.detail(id)` used a completely different key prefix (`["store", id]`) than `storesQueryKeys.root` (`["stores"]`), so every Store mutation's `invalidateQueries({ queryKey: storesQueryKeys.root })` (used by create/update/delete, and by the distribution-rules and organizations hooks too) never actually refetched an open `useStore(id)` query — the exact query `BrandingSettingsForm` depends on for `logo_document`/`cover_document`. Fixed by nesting `detail` under the same `"stores"` root key. Live-verified as a side effect of the document-remove test above: the logo and its "Remove" button both disappeared immediately after the mutation with no manual refresh, which would not have happened before this fix.

Everything audited that turned out **not** to be a gap, so nothing was built for it: `GET /google-maps/timezone` (the frontend only ever collects a free-text address, not lat/lng, and no geocoding endpoint exists to bridge the two — wiring this would mean inventing a geocode step the API doesn't provide, not filling a defined gap); `PATCH`/`DELETE /organizations/:id` (this product deliberately hides "Organization" as a concept from the single-store UX throughout — per 2.1's "Org jargon hidden" — so no rename/delete-business UI is a deliberate omission, not a miss); `PATCH /stores/:storeId/payout-account` (its own Swagger summary says it's "Mock-update... to simulate RESTRICTED/DISABLED" — a testing utility, not a real Owner-facing action).

All backend tests pass (149 suites / 1078 tests, +3 from this round), both packages typecheck clean, lint is clean except the same 3 pre-existing unrelated errors already documented in Phase 9, and `next build` still succeeds.
...
| Phase 7 | 6 | 6 (empty states, loading skeletons, mutation toasts, unsaved-changes, demo-data purge, and the responsive pass — all live-verified 2026-08-29) |
...
| **Overall (0–10)** | **87** (now fully recounted, replacing every earlier estimate) | **78** (+1 from the 2026-08-29 live-testing round: Phase 7's responsive pass went from "static review only" to live-verified) |
